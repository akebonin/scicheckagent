...
def generate_questions_for_claim(claim):
    ...
    prompt = f"For the following claim, propose up to 3 concise research questions. Output ONLY a numbered list. Do not include any other text.\n\nClaim: {claim}"
    ...
    try:
        ...
        questions = []
        for line in lines:
            # Use regex to find lines that start with a number, dot, and optional space
        ...
        # Fallback for cases where AI doesn't use numbers (e.g., uses hyphens or just newlines)
        if not questions:
            questions = [q.strip("-•* ") for q in lines if q.strip() and len(q.strip()) > 10]
...
def fetch_crossref(keywords):
    ...
    url = f"[https://api.crossref.org/works?query=](https://api.crossref.org/works?query=){quote_plus(search_query)}&rows=3&select=title,URL,author,abstract"
    headers = {"User-Agent": "SciCheckAgent/1.0 (mailto:alizgravenil@gmail.com)"}
...
def fetch_core(keywords):
    ...
    # Join keywords with AND, add quotes for multi-word phrases for better search specificity
    search_query = ' AND '.join([f'"{kw}"' if ' ' in kw else kw for kw in keywords])
    url = f"[https://core.ac.uk:443/api-v2/search/](https://core.ac.uk:443/api-v2/search/){quote_plus(search_query)}?page=1&pageSize=3&metadata=true"
...
def fetch_pubmed(keywords):
    ...
    url = f"[https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=](https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=){search_query}&retmode=json&retmax=3"
    ...
    try:
        ...
        details_url = f"[https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=](https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=){','.join(id_list)}&retmode=json"
        ...
        for pubmed_id in id_list:
            ...
            results.append({
                ...
                "url": f"[https://pubmed.ncbi.nlm.nih.gov/](https://pubmed.ncbi.nlm.nih.gov/){pubmed_id}/"
            })
...
def export_pdf():
    ...
    for item in pdf_reports:
        ...
        if item.get('report'):
            ...
            y = draw_markdown_content(p, item['report'], styles, y, width, left_margin=inch, right_margin=inch)
        y -= 20
...
def draw_markdown_content(pdf_canvas, markdown_text, styles, y_pos, page_width, left_margin=0.75*inch, right_margin=0.75*inch):
    available_width = page_width - left_margin - right_margin
    elements = []
    ...
    for line in lines:
        ...
        if is_table_line:
            if not in_table:
                # Flush text buffer before starting a table
                if text_buffer:
                    elements.append(Paragraph("<br/>".join(text_buffer), styles['ReportBody']))
                    text_buffer = []
                in_table = True
            ...
            # Wrap each cell content in a Paragraph for better formatting and line breaks
            formatted_row = [Paragraph(cell, styles['NormalParagraph']) for cell in row]
            table_data.append(formatted_row)
        else:
            if in_table:
                # Table ended, create and add Table object
                if table_data:
            ...
            # Add non-table line to buffer
            if line:
                # Handle markdown formatting for text
                line = re.sub(r'\*\*(.*?)\*\*', r'<b>\1</b>', line)
                line = re.sub(r'\[(.*?)\]\((https?://[^\s\]]+)\)', r'<link href="\2">\1</link>', line)
                text_buffer.append(line)

    # Flush remaining text or table data
    if text_buffer:
        elements.append(Paragraph("<br/>".join(text_buffer), styles['ReportBody']))
    ...
    for element in elements:
        w, h = element.wrapOn(pdf_canvas, available_width, 0)
        if y_pos - h < 0.75 * inch:
            pdf_canvas.showPage()
            y_pos = A4[1] - 0.75 * inch
        element.drawOn(pdf_canvas, left_margin, y_pos - h)
        y_pos -= (h + styles['ReportBody'].spaceAfter)
