...
<head>
    ...
    <script type="text/javascript" src="[https://cs.iubenda.com/autoblocking/4149829.js](https://cs.iubenda.com/autoblocking/4149829.js)"></script>
    <script type="text/javascript" src="//[cdn.iubenda.com/cs/tcf/stub-v2.js](https://cdn.iubenda.com/cs/tcf/stub-v2.js)"></script>
    <script type="text/javascript" src="//[cdn.iubenda.com/cs/tcf/safe-tcf-v2.js](https://cdn.iubenda.com/cs/tcf/safe-tcf-v2.js)"></script>
    <script type="text/javascript" src="//[cdn.iubenda.com/cs/gpp/stub.js](https://cdn.iubenda.com/cs/gpp/stub.js)"></script>
    <script type="text/javascript" src="//[cdn.iubenda.com/cs/iubenda_cs.js](https://cdn.iubenda.com/cs/iubenda_cs.js)" charset="UTF-8" async></script>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>SciCheck AI Agent</title>
    <script async src="[https://www.googletagmanager.com/gtag/js?id=G-H5SNSXX9K0](https://www.googletagmanager.com/gtag/js?id=G-H5SNSXX9K0)"></script>
    <script>
    ...
    <style>
        ...
        .report-box {
            ...
            border-color: #cce7ff;
            margin-top: 15px;
            white-space: normal; /* Allow normal wrapping for rendered HTML */
        }
        .report-placeholder {
            min-height: 50px;
            padding-left: 15px;
        ...
        .form-check-input.form-switch {
            ...
            margin-left: 0.5em;
            vertical-align: middle;
            background-image: url("data:image/svg+xml,%3csvg xmlns='[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)' viewBox='-4 -4 8 8'%3e%3ccircle r='3' fill='rgba%280, 0, 0, 0.25%29'/%3e%3c/svg%3e");
        ...
        .form-check-input.form-switch:checked {
            ...
            background-image: url("data:image/svg+xml,%3csvg xmlns='[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)' viewBox='-4 -4 8 8'%3e%3ccircle r='3' fill='%23fff'/%3e%3c/svg%3e");
        }
        ...
        .report-box .table {
            font-family: 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; /* Use body font for tables */
            font-size: 0.9rem;
        }
    </style>
</head>
<body>
    <div class="container">
        <header class="header">
            <img src="[https://raw.githubusercontent.com/akebonin/scicheckagent/refs/heads/main/logo.png](https://raw.githubusercontent.com/akebonin/scicheckagent/refs/heads/main/logo.png)" alt="SciCheck Logo">
            ...
            <form id="analysis-form">
                ...
                <div id="url-input-group" class="mb-3 d-none">
                    ...
                    <div class="input-group">
                        <input id="urlInput" type="text" class="form-control" placeholder="[https://example.com](https://example.com)">
                ...
                <div id="video-url-input-group" class="mb-3 d-none">
                    ...
                    <div class="input-group">
                        <input id="videoUrlInput" type="text" class="form-control" placeholder="[https://youtube.com/watch?v=](https://youtube.com/watch?v=)... or [https://vimeo.com/](https://vimeo.com/)...">
        ...
        <footer class="text-center text-muted footer">
            Built by <a href="[https://alizgravenil.wixsite.com/alisgravenil](https://alizgravenil.wixsite.com/alisgravenil)" target="_blank" rel="noopener noreferrer">Alis Grave Nil</a>
        </footer>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function renderMarkdown(text) {
                    ...
                    if (line.trim().startsWith('|')) {
                        if (!inTable) {
                            ...
                            if (nextLine.match(/^\|\s*[-:]+\s*\|/)) {
                                const headers = line.trim().slice(1, -1).split('|');
                                html += '<thead><tr>';
                        ...
                        // Apply other markdown rules for non-table lines
                        if (line.trim() === '') {
                        ...
                        if (line.startsWith('#')) {
                            ...
                            html += `<h${level+2}>${escapeHTML(content)}</h${level+2}>`; // h3, h4, etc.
                        } 
                        // Numbered/Unordered list items
                        else if (line.match(/^(\d+\.|-|\*)\s/)) {
                            let p_line = escapeHTML(line);
                            p_line = p_line.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
                            p_line = p_line.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer">$1</a>');
                            html += `<p>${p_line}</p>`;
                        }
                        // Default paragraph
                        else {
                ...
                if (inTable) {
                    html += '</tbody></table>'; // Close table if it's the last element
                }
            ...
            async function autoLoadClaimDetails(claims) {
                const promises = claims.map((claim, index) => {
                    const claimId = `claim-${index}`;
            ...
            async function getModelDetails(claimIdx, verdictContainerId, questionsContainerId, button, autoLoad = false) {
                ...
                try {
                    ...
                    verdictContainer.innerHTML = renderMarkdown(data.model_verdict);
            ...
            async function verifyExternal(claimIdx, verdictContainerId, sourcesContainerId, button, autoLoad = false) {
                ...
                try {
                    ...
                    const data = await response.json();
                    verdictContainer.innerHTML = renderMarkdown(data.verdict);
                        ...
                        for (const line of lines) {
                            if (line.startsWith('data: ')) {
                                ...
                                try {
                                    ...
                                    if (jsonData.content) {
                                        fullReportText += jsonData.content;
                                    }
                                    if (jsonData.error) {
                                        throw new Error(jsonData.error);
                                    }
                                } catch (e) {
                                    console.warn("Error parsing SSE chunk:", e, "Chunk:", dataPart);
                                }
                            }
                        }
                    }

                    // Render the entire report at the end to correctly apply markdown
                    reportContainer.innerHTML = `<div class="report-box">${renderMarkdown(fullReportText)}</div>`;
                    updatePdfSelectionList();
            ...
            fetchArticleBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (!data.article_text) throw new Error('No content was extracted.');

                    textInput.value = data.article_text;
                    inputMethodSelect.value = 'paste';
                    inputMethodSelect.dispatchEvent(new Event('change'));
                    alert('Article content has been fetched and pasted.');
            ...
            analysisForm.addEventListener('submit', async (e) => {
                ...
                if (!articleText) {
                    alert('Article text is empty. Please paste content or use one of the other input methods.');
                    return;
                }
                ...
                try {
                    ...
                    const data = await response.json();
                    if (!response.ok) throw new Error(data.error || `Failed to analyze text: ${response.status}`);
            ...
            selectAllCheckbox.addEventListener('change', (e) => {
                document.querySelectorAll('.report-selection').forEach(checkbox => {
